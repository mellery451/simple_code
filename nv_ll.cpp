...
vector<int> run_test(string operation, vector<int> context) {
    ...
    if (operation == "create_from_vector") {
        ...
        for (int i = 0; i < numRemoves && (2 + size + i) < static_cast<int>(context.size()); i++) {
 ...
 string ltrim(const string &str) {
     ...
     s.erase(
         s.begin(),
         find_if(s.begin(), s.end(), [](const unsigned char ch){return ! std::isspace(ch);})
     );
 ...
 string rtrim(const string &str) {
     ...
     s.erase(
         find_if(s.rbegin(), s.rend(), [](const unsigned char ch){return ! std::isspace(ch);}).base(),
         s.end()
     );
